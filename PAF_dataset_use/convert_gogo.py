...
def load_file_descriptions(data_root):
    ...
    """
    description_paths = {
        'train': os.path.join(data_root, 'description', 'PAF_train.txt'),
        'val': os.path.join(data_root, 'description', 'PAF_val.txt'),
    }
    with open(os.path.join(data_root, 'description', 'PAF.json')) as f:
...
def process_dataset(converter: BaseEventImageConverter, file_extension, num_workers=None):
    ...
    data_root = "/media/2TB_1/dataset/PAF" if sys.platform == 'linux' else r"E:/dataset/PAF"
    out_root = "/media/2TB_1/Bacon/dataset/PAF" if sys.platform == 'linux' else r"E:/dataset/PAF"

    file_list = glob.glob(fr"{data_root}/PAF_aedat/*/*.{file_extension}")
    ...
    for item in file_list:
        ...
        file_name = re.findall(r"PAF_aedat/(.*?\.aedat)", item)[0]
